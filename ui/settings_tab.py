...
class SettingsTab(ttk.Frame):
    ...
    def _setup_ui(self):
        ...
        # Dùng Frame rỗng để đẩy nút xuống đáy nếu cần, hoặc để grid tự nhiên
        ttk.Separator(left_frame, orient='horizontal').grid(row=8, column=0, sticky="ew", pady=20)
        
        ttk.Button(left_frame, text="💾 LƯU CẤU HÌNH", style="Accent.TButton", command=self.save_settings).grid(row=9, column=0, sticky="ew", pady=5)
        ttk.Button(left_frame, text="🔄 Mặc định", command=self.reset_defaults).grid(row=10, column=0, sticky="ew")
